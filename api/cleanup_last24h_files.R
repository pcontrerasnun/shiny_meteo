...
clean_files <- function(station) {
  ...
  for (file in file_list) {
    ...
    file_hour <- substr(basename(file), 10, 13)
    ...
    else if (file_date < current_date & !file_hour %in% c("0000","1200")) {
      # Obtener el fichero más reciente para este día
      todays_files <- list.files(path = file_path, pattern = paste0(format(current_date, "%Y%m%d"),"*last24h.csv.gz"), full.names = TRUE)
