...
MonthlyTmeanAnomaliesPlot <- function(data, selected_year, ref_start_year, ref_end_year, max_date) {
  ...
  reference_monthly_tmean <- data_temp |>     
    ...
    dplyr::summarise(tmean_ref = round(mean(tmean, na.rm = TRUE), 1)) |> 
    dplyr::as_tibble()
  ...
  plot_data <- dplyr::left_join(reference_monthly_tmean, selected_year_tmean, by = "month") |> 
    dplyr::mutate(tano = round(tmean - tmean_ref, 1)) |> 
    dplyr::mutate(tano_label = ifelse(tano > 0, paste0("+", tano), tano)) |> 
