CumPcpPlot <- function(data, selected_year, ref_start_year, ref_end_year) {
  ...
  plot_data <- left_join(mean_pcp, selected_year_pcp, by = c("dia", "mes")) |>
    dplyr::select(
  ...
  q <- ggplot2::ggplot(data = plot_data, aes(x = fecha, y = cumsumpcp)) +
    ...
    ggplot2::scale_y_continuous(
      ...
      breaks = seq(from = 0, to = max(plot_data$cummeanpcp) + 400, by = 100), expand = c(0, 20, 0, 100)
    ) +
    ggthemes::theme_hc(base_size = 15) +
    labs(
      x = "", y = "", title = paste0("Precipitación en Madrid - Retiro ", selected_year),
      subtitle = paste0("Precipitación acumulada comparada con valores históricos (", 
                        ref_start_year, "-", ref_end_year, ")"),
      caption = paste0("Actualizado: ", max(plot_data$fecha, na.rm = TRUE), "
                       , Fuente: AEMET, Elab. propia (@Pcontreras95)")
    ) +
    ggplot2::theme(
      plot.title = element_text(hjust = 1, face = "bold", family = "sans", size = 35),
      plot.subtitle = element_text(hjust = 1, size = 25), legend.position = "none"
    )
