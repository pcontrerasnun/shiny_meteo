DailySunsetGainedPlot <- function(data, selected_year, max_date, title) {
  plot_data <- data |> 
    dtplyr::lazy_dt() |>
    dplyr::mutate(sunset_time = hms::as_hms(format(sunset, "%H:%M:%S"))) |> 
    dplyr::mutate(diff_sunset = difftime(sunset_time, lag(sunset_time), units = "mins")) |> 
    dplyr::mutate(diff_sunset = ifelse(abs(diff_sunset) > 30, diff_sunset - 60 * sign(diff_sunset), diff_sunset)) |> # Por los cambios de hora de verano e invierno
    dplyr::filter(date >= as.Date(paste0(selected_year, "-01-01")) & # Filter after mutate so than Jan 1 is not NA
                    date <= as.Date(paste0(selected_year, "-12-31"))) |>
    dplyr::select(date, diff_sunset) |> 
    dplyr::as_tibble()
  ...
  color_data <- plot_data |> 
    dplyr::reframe(x = seq(min(as.Date(plot_data$date), na.rm = TRUE), max(as.Date(plot_data$date), na.rm = TRUE), length = 1000),
                   y1 = approx(as.Date(date), diff_sunset, xout = x)$y,
                   diff = 0 - y1)
  
  p <- ggplot2::ggplot(plot_data, aes(x = date, y = diff_sunset)) + 
    ...
    ggrepel::geom_label_repel(
      ...
      aes(y = diff_sunset, label = round(diff_sunset, 2))
    ) +
    ggplot2::scale_color_gradient2(high = "#172869", mid = "#EFC7E6", low = "#B25D91", guide = guide_none()) +
    ...
    ggplot2::labs(
      x = "", y = "", title = paste0("Sunset in ", title, " ", selected_year),
      subtitle = "Gained daylight minutes at night (daily change)",
      caption = paste0(
  ...
  return(list(p, plot_data, "date", "diff_sunset"))
