MonthlyHistoricalMinTmaxPlot <- function(data, ref_start_year, ref_end_year, max_date, title) {
  ...
  ranking_maxtmaxs <- plot_data |> 
    ...
    dplyr::mutate(ranking = rank(-mintmax, ties.method = "first")) |>
  ...
  ranking_mintmaxs <- plot_data |> 
    ...
    dplyr::arrange(-mintmax) |> 
    dplyr::group_by(month) |> 
    dplyr::mutate(ranking = rank(mintmax, ties.method = "last")) |>
