MonthlyPcpPlot <- function(data, selected_year, ref_start_year, ref_end_year, max_date, title) {
...
#    ) +
    ...
    ggplot2::scale_y_continuous(
      ...
      breaks = seq(from = 0, to = max(max(plot_data$p100pcp), max(plot_data$sumpcp, na.rm = TRUE))
                   + max(max(plot_data$p100pcp), max(plot_data$sumpcp, na.rm = TRUE)) * 0.66, by = 50), # add 2/3 of max pcp value for legend
      limits = c(0, max(max(plot_data$p100pcp), max(plot_data$sumpcp, na.rm = TRUE))
                 + max(max(plot_data$p100pcp), max(plot_data$sumpcp, na.rm = TRUE)) * 0.66)) + # add 2/3 of max pcp value for legend
