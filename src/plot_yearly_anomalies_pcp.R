...
AnualPcpAnomaliesPlot <- function(data, ref_start_year, ref_end_year, max_date) {
  # Calculate median precipitation in reference period
  reference_anual_pcts_pcp <- data |>
    dtplyr::lazy_dt() |>
  ...
  # Join data
  plot_data <- cbind(anual_pcp, reference_anual_pcts_pcp) |> 
  ...
  p <- ggplot2::ggplot(data = plot_data, aes(x = year, y = pcp)) +
    ...
    ggplot2::scale_linetype_manual(values = c("q50" = "longdash", "trend" = "dotted"),
                                   labels = c(paste0("Normal precip. (", ref_start_year, "-", ref_end_year, ")"),
                                              "Trend")) +
