DailyTmeanPlot <- function(data, data_forecast, selected_year, ref_start_year, ref_end_year, max_date, title) {
  ...
  reference_daily_pcts_tmean <- data |> 
    ...
    dplyr::summarise(
      p00tmean = round(quantile(climate_tmean, probs = 0.00, na.rm = TRUE), 1), # not plotting but interesting to see in data
      p01tmean = round(quantile(climate_tmean, probs = 0.01, na.rm = TRUE), 1),
      ...
      p99tmean = round(quantile(climate_tmean, probs = 0.99, na.rm = TRUE), 1),
      p100tmean = round(quantile(climate_tmean, probs = 1, na.rm = TRUE), 1), # not plotting but interesting to see in data
  ...
  p <- ggplot2::ggplot(data = plot_data, aes(x = date)) +
    ...
    ggplot2::geom_ribbon(aes(ymin = p95tmean, ymax = p99tmean, fill = "P100"), # p95tmean + 4
    ...
    ggplot2::geom_ribbon(aes(ymin = p05tmean, ymax = p20tmean, fill = "P20"),
                         ...
                         lineend = "round", linejoin = "round") +
    ggplot2::geom_ribbon(aes(ymin = p01tmean, ymax = p05tmean, fill = "P00"), # p05tmean - 4
