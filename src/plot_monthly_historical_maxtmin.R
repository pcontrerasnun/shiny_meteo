MonthlyHistoricalMaxTminPlot <- function(data, ref_start_year, ref_end_year, max_date) {
  ...
  plot_data <- data |> 
    ...
    dplyr::summarise(maxtmin = max(tmin, na.rm = TRUE), .groups = "keep") |> 
    dplyr::filter(maxtmin != -Inf) |> 
    dplyr::as_tibble()
  ...
  # Draw the plot
  p <- ggplot2::ggplot(data = plot_data, aes(x = year, y = maxtmin)) +
    ggplot2::geom_line(aes(color = "maxtmin")) +
    ...
    ggplot2::scale_color_manual(
      values = c("trend" = "blue", "maxtmin" = "black"), 
  ...
  return(list(p, plot_data, "year", "maxtmin"))
